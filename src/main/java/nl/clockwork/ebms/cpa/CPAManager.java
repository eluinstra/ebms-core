...
public class CPAManager
{
	...
	{
		return collaborationRole -> toParty == null
				|| toParty.matches(collaborationRole.getRole()) && service.equals(CPAUtils.toString(collaborationRole.getServiceBinding().getService()));
	}
	...
	{
		return getCPA(cpaId)
				.flatMap(cpa -> cpa.getPartyInfo().stream().flatMap(partyInfo -> partyInfo.getPartyId().stream()).filter(matchesPartyId(partyId)).findAny())
				.isPresent();
	}
	...
	{
		return getCPA(cpaId).flatMap(
				cpa -> cpa.getPartyInfo()
						.stream()
						.filter(partyInfo -> partyInfo.getPartyId().stream().anyMatch(matchesPartyId(partyId)))
						.map(partyInfo -> createEbMSPartyInfo(partyId, partyInfo).orElseThrow(() -> new IllegalStateException("PartyId " + partyId + " not found")))
						.findFirst());
	}
	...
	{
		return getCPA(cpaId).flatMap(cpa -> cpa.getPartyInfo().stream().filter(matchesPartyInfo(partyId)).findFirst());
	}
	...
	{
		return getFromPartyInfo(cpaId, fromParty, service, action).flatMap(
				fromPartyInfo -> getCPA(cpaId).flatMap(
						cpa -> cpa.getPartyInfo()
								.stream()
								.flatMap(
										partyInfo -> partyInfo.getCollaborationRole()
	...
	{
		return getCPA(cpaId)
				.flatMap(cpa -> cpa.getPartyInfo().stream().flatMap(streamDeliveryChannels()).filter(matchesDeliveryChannel(deliveryChannelId)).findFirst());
	}
	...
	{
		return getPartyInfo(cpaId, partyId).map(
				partyInfo -> partyInfo.getOverrideMshActionBinding()
						.stream()
	...
	public Optional<DeliveryChannel> getSendDeliveryChannel(String cpaId, List<PartyId> partyId, String role, String service, String action)
	{
		return EbMSAction.EBMS_SERVICE_URI.equals(service)
				...
				: getPartyInfo(cpaId, partyId).flatMap(
						partyInfo -> partyInfo.getCollaborationRole()
								.stream()
								.filter(matchesRoleByRole(role).and(matchesRoleByService(service)))
								.flatMap(streamCanSends())
