...
class EbMSMessageWriter
{
	...
	{
		return document.getAttachments().isEmpty()
				? writeMessage(request, document)
				: writeMimeMessage(request, document);
	}
	...
	protected Builder writeMimeMessage(Builder request, EbMSDocument document) throws TransformerException
	{
		...
		val publisher = new MultipartBodyPublisher(document.getContentId())
				.addXml(document.getContentId(),message);
		document.getAttachments().stream().forEach(publisher::addAttachment);
		return request
				.setHeader("Content-Type",publisher.contentType())
				.setHeader("SOAPAction",Constants.EBMS_SOAP_ACTION)
				.POST(publisher);
	}
}
