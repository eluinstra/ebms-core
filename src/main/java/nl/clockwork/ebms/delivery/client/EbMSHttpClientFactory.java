	...
	private EbMSClient createEbMSClient(String clientAlias)
	{
		try
		{
			...
			if (EbMSHttpClientType.APACHE.equals(type))
				return new nl.clockwork.ebms.delivery.client.apache.EbMSHttpClient(sslFactoryManager,enabledProtocols,enabledCipherSuites,verifyHostnames,connectTimeout,chunkedStreamingMode,proxy);
			else
