...
public class EbMSMessageContentSerializer extends AbstractMessageAwareTransformer
{
	...
	public Object transform(MuleMessage message, String outputEncoding) throws TransformerException
	{
		try
		{
			...
			message.setPayload(new nl.clockwork.mule.ebms.bridge.tcp.model.EbMSMessageContent(content.getContext() == null ? null : new EbMSMessageContext(content.getContext().getCpaId(),content.getContext().getFromRole(),content.getContext().getToRole(),content.getContext().getService(),content.getContext().getAction(),content.getContext().getConversationId(),content.getContext().getMessageId(),content.getContext().getRefToMessageId()),content.getProperties(),attachments));
			return message;
		}
		catch (IOException e)
		{
