 ...
 ******************************************************************************/
package nl.clockwork.mule.ebms.dao.hsqldb;
										...
										try
										{
											...
											(
												"insert into ebms_message (" +
													...
													"status_date," +
													"next_retry_time" +
												") values (?,?,?,(select COALESCE(max(sequence_nr) + 1,0) from ebms_message where cpa_id=? and conversation_id=?),?,?,?,?,?,?,?,NOW(),?)",
												//new String[]{"id"}
												new int[]{1}
											);
											...
											ps.setString(4,cpaId);
											ps.setString(5,conversationId);
											ps.setString(6,messageId);
											ps.setInt(7,messageType.id());
											ps.setBytes(8,messageOriginal);
											ps.setString(9,XMLMessageBuilder.getInstance(MessageHeader.class).handle(messageHeader));
											ps.setString(10,ackRequested == null ? null : XMLMessageBuilder.getInstance(AckRequested.class).handle(ackRequested));
											ps.setString(11,manifest == null ? null : XMLMessageBuilder.getInstance(Manifest.class).handle(manifest));
											ps.setInt(12,status.id());
											ps.setString(13,nextRetryTime == null ? null : String.format(defaultDateFormat,nextRetryTime));
											return ps;
										}
										catch (JAXBException e)
										{
