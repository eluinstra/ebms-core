/*
 * $Id: MuleMessageToHttpResponse.java 14181 2009-02-27 20:19:51Z aperepel $
 * --------------------------------------------------------------------------------------
    ...
    public Object transform(MuleMessage msg, String outputEncoding) throws TransformerException
    {
        ...
        try
        {
            ...
            if (fields.length == 2)
            {
	            ...
	            if (fields[0].getType().equals(MessageImpl.class))
	            try
							{
	            	MessageImpl map = (MessageImpl)fields[0].get(src);
	            	MuleMessage m = (MuleMessage)map.get("mule.message");
	            	exception = m.getExceptionPayload() != null;
//								Field field = map.getClass().getDeclaredField("contents");
//  	            field.setAccessible(true);
//  	            Map<Class<?>, Object> contents = (Map<Class<?>,Object>)field.get(map);
//  	            exception = contents != null && contents.containsKey(java.lang.Exception.class);
							}
							catch (Exception e)
							{
							}
          	...
          	if (exception)
            	response.setStatusLine(response.getHttpVersion(),HttpConstants.SC_INTERNAL_SERVER_ERROR);
          	else
          		if(!(src instanceof String))
            //if (msg.getProperty(HttpConnector.HTTP_STATUS_PROPERTY) != null && HttpConstants.SC_NO_CONTENT == (Integer)msg.getProperty(HttpConnector.HTTP_STATUS_PROPERTY))
            if (msg.getProperty("EBMS.HTTP_STATUS_CODE") != null && HttpConstants.SC_NO_CONTENT == (Integer)msg.getProperty("EBMS.HTTP_STATUS_CODE"))
            {
            	response.setStatusLine(response.getHttpVersion(),HttpConstants.SC_NO_CONTENT);
            	response.setHeaders(new Header[]{});
            	response.setBody((OutputHandler)null);
            }
          	//PATCH_END
