...
class JMSDeliveryTaskManager implements DeliveryTaskManager
{
	...
	public void updateTask(DeliveryTask task, String url, DeliveryTaskStatus status, String errorMessage)
	{
		...
		{
			val nextTask = createNextTask(task, createTimestamp(task, deliveryChannel));
	...
	private Instant createTimestamp(DeliveryTask task, DeliveryChannel deliveryChannel)
	{
		...
		return task.getRetries() < rm.getRetries().intValue() ? Instant.now().plus(rm.getRetryInterval()) : task.getTimeToLive();
	}
