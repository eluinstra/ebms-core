 ...
 */
package nl.clockwork.ebms.plugin.messaging.kakfa.config;
...
import nl.clockwork.ebms.common.deliverytask.KafkaTaskHandlerType;
...
public class DeliveryTaskHandlerConfig
{
	...
	{
		log.info("Initializing deliveryTaskKafkaTemplate");
	...
	{
		log.info("Initializing messagePropertiesKafkaTemplate");
	...
	{
		val result = new KafkaTransactionManager<>(deliveryTaskProducerFactory());
	...
	{
		val result = new KafkaTransactionManager<>(messagePropertiesProducerFactory());
		result.setTransactionSynchronization(AbstractPlatformTransactionManager.SYNCHRONIZATION_ON_ACTUAL_TRANSACTION);
		return result;
	}
}
