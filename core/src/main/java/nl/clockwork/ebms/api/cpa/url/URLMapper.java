...
public class URLMapper
{
	...
	{
		return repository.getURLMappings();
	}

	public String getURL(String source)
	{
		if (!StringUtils.isEmpty(source))
			return repository.getURLMapping(source).orElse(source);
		else
			return source;
	}
	...
	public void setURLMapping(URLMapping urlMapping)
	{
		synchronized (urlMonitor)
		{
			if (StringUtils.isEmpty(urlMapping.getDestination()))
				repository.deleteURLMapping(urlMapping.getSource());
			else
	...
	{
		if (repository.existsURLMapping(urlMapping.getSource()))
			repository.updateURLMapping(urlMapping);
		else
			repository.insertURLMapping(urlMapping);
	}

	public int deleteURLMapping(String source)
	{
		return repository.deleteURLMapping(source);
	}
